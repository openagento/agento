...
| `agento run <code> "<prompt>"` | **Headless (one-shot)** | Runs the agent with the given prompt, streams output to your terminal, then exits with the agent's exit code (`docker compose exec -T`). Stdin is closed. Add `--pretty` for a readable stream. |
...
The agent runs inside the isolated `sandbox` container with no upstream service credentials of its own (the toolbox is designed to be the only container that holds them; known gaps: [zero-trust.md](../architecture/zero-trust.md#known-exceptions-and-debt)), and its toolbox capability is scoped to that one agent_view and expires. Concurrent runs share the `agent` UID and the workspace mount, so this is not a boundary between mutually hostile views — see the co-tenant limit in [zero-trust.md](../architecture/zero-trust.md). `--yolo` only affects **interactive** mode — headless (one-shot) runs are always in bypass mode, so the flag is a no-op there.
