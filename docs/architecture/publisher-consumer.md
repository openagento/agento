...
| `BLOCKED` | Halted by a *blocked* verification verdict — a deterministic configuration/infrastructure fault (e.g. a missing MCP credential) that cannot heal on retry. No retry, no dead-letter; an admin alert fires instead (`job_blocked_after`). |
