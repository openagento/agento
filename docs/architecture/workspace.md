...
No awareness of other jobs. The run holds the agent's own harness credential and its own scoped toolbox capability. The target model gives it no upstream service credentials and no direct DB access; a headless run today inherits the cron env (see [zero-trust.md](zero-trust.md#known-exceptions-and-debt)).
