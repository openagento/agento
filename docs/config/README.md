...
- **Toolbox (Node):** [config-loader.js](../../src/agento/toolbox/config-loader.js) — a deliberately separate mirror (the toolbox is designed to be the only container that holds tool credentials, with one documented exception: the SSH private key used for git — see [DECISIONS.md](../../DECISIONS.md) D-SSH-1; known gaps: [zero-trust.md](../architecture/zero-trust.md#known-exceptions-and-debt)). Kept behaviorally in sync; not merged with the Python service.
