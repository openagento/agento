...
Regression for a race in ``select_credential``: ``SELECT ... LIMIT 1 FOR UPDATE
...
def _run_claim_burst(int_db_config, resolver: CredentialResolver) -> tuple[list[str | None], list[str]]:
    ...
    def worker(idx: int) -> None:
        ...
        try:
            ...
            token = resolver.resolve(conn, "claude")
