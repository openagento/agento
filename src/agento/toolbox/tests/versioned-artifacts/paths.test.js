...
import { safeJoin, safeResolve, validateArtifactCode, validateDraftId, validateVersionId } from '../../../modules/versioned_artifacts/toolbox/paths.js';

const ROOT = '/srv/versioned-artifacts/site/worktrees/d-a83f21';
...
describe('safeJoin — syntactic containment', () => {
  it.each([
    ...
    '', '.', '..',
  ])('rejects %j', (bad) => { expect(() => safeJoin(ROOT, bad)).toThrow(/INVALID_PATH|PATH_OUTSIDE_ARTIFACT/); });
  ...
  it('rejects a sibling sharing the root prefix', () => {
    expect(() => safeJoin(ROOT, '../d-a83f21-evil/x')).toThrow(/INVALID_PATH|PATH_OUTSIDE_ARTIFACT/);
  });
});
...
describe('identifier validation', () => {
  it.each(['openagento-website', 'a', 'monthly-sales-report'])('accepts artifact_code %j', (c) => {
    expect(validateArtifactCode(c)).toBe(c);
  });
  it.each(['../evil', '/abs', 'Has-Upper', 'has_underscore', '-leading', '', 'x'.repeat(65), 'a/b'])(
    'rejects artifact_code %j', (c) => { expect(() => validateArtifactCode(c)).toThrow(/INVALID_PATH/); });
