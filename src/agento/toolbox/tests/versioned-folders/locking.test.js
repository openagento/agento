...
import { withLock, sweepStaleLocks } from '../../../modules/versioned_folders/toolbox/locking.js';
...
it('a heartbeat keeps a live holder\'s lock fresh so the sweep spares it', async () => {
  const lock = path.join(dir, 'site', 'locks', 'e.lock');   // the REAL layout: <root>/<folder_code>/locks/
