...
export async function main(argv, payload, deps) {
  ...
  // `admin: true` — the SAME exemption `init` has always carried, and the reason the
  // other two operations need it: `allowed_artifacts` is agent_view-scoped and empty by
  // default, while this CLI resolves DEFAULT scope, so `list` would print nothing and
  ...
  try {
    switch (argv.op ?? 'init') {
      case 'init':
        return await service.init(payload.artifact_code, {
          files: payload.files, title: payload.title ?? null, owner: payload.owner ?? null,
        });
      case 'list':
        return { artifacts: await service.listArtifacts() };
      case 'publish':
      ...
      default:
        ...
        return { error_code: ERROR_CODES.INVALID_OPERATION, message: 'unknown operation' };
    }
  } catch (err) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  try {
    ...
    process.stdout.write(`${JSON.stringify({
      error_code: ERROR_CODES.STORAGE_OPERATION_FAILED, message: 'the toolbox command failed',
    })}\n`);
    process.exit(1);
  }
}
