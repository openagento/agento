...
export const ARTIFACT_CODE_RE = /^[a-z0-9][a-z0-9-]{0,63}$/;
...
export const validateArtifactCode = (v) => check(ARTIFACT_CODE_RE, v, 'artifact_code');
...
/** A selector names exactly ONE source, and this returns the id it named. One rule in
 *  one place: the tool layer, the service and the backend all derive a NAME from a
 *  selector — a desk directory, a lock file, a treeish — and three copies of the check
 *  is three chances for them to disagree about which source a malformed selector meant. */
export const selectorSourceId = (selector) => {
  const hasDraft = selector?.draftId != null;
  const hasVersion = selector?.versionId != null;
  if (hasDraft === hasVersion) {
    throw new ArtifactError(ERROR_CODES.INVALID_PATH,
      hasDraft ? 'name either a draft or a version, not both' : 'a draft or a version must be named');
  }
  return hasDraft ? validateDraftId(selector.draftId) : validateVersionId(selector.versionId);
};
...
export function validateRelPath(relPath) {
  if (typeof relPath !== 'string' || relPath === '') throw new ArtifactError(ERROR_CODES.INVALID_PATH, 'path must be a non-empty string');
  if (path.isAbsolute(relPath) || /^[\\/]/.test(relPath)) throw new ArtifactError(ERROR_CODES.INVALID_PATH, 'absolute paths are not allowed');
  ...
  // so say so.
  if (relPath.includes('\\')) throw new ArtifactError(ERROR_CODES.INVALID_PATH, 'paths use forward slashes');
  const segments = segmentsOf(relPath);
  if (segments.length === 0) throw new ArtifactError(ERROR_CODES.INVALID_PATH, 'path must name a file');
  if (segments.includes('..')) throw new ArtifactError(ERROR_CODES.INVALID_PATH, 'parent traversal is not allowed');
  if (segments[0] === '.git') throw new ArtifactError(ERROR_CODES.INVALID_PATH, 'internal metadata is not accessible');
  if (segments.some((s) => s.includes('\0'))) throw new ArtifactError(ERROR_CODES.INVALID_PATH, 'invalid character in path');
...
export function safeJoin(root, relPath) {
  ...
  if (resolved !== resolvedRoot && !resolved.startsWith(resolvedRoot + path.sep)) {
    throw new ArtifactError(ERROR_CODES.PATH_OUTSIDE_ARTIFACT, 'path escapes the artifact');
  }
  return resolved;
}
...
export async function safeResolve(root, relPath, _opts = {}) {
  ...
  for (const segment of path.relative(resolvedRoot, resolved).split(path.sep)) {
    ...
    try { st = await lstat(current); }
      ...
      throw new ArtifactError(ERROR_CODES.INVALID_PATH, 'path could not be verified', { cause: err });
    }
    if (st.isSymbolicLink()) throw new ArtifactError(ERROR_CODES.SYMLINK_NOT_ALLOWED, 'symbolic links are not allowed in this artifact');
  }
  return resolved;
}
...
export const artifactRoot = (storageRoot, code) => path.join(storageRoot, validateArtifactCode(code));
export const repoDir = (storageRoot, code) => path.join(artifactRoot(storageRoot, code), 'repo.git');
export const draftDir = (storageRoot, code, draftId) =>
  path.join(artifactRoot(storageRoot, code), 'worktrees', validateDraftId(draftId));
