...
import { validateFolderCode, listDirOrEmpty } from './paths.js';
...
export async function withLock(lockPath, fn, { timeoutMs = 10_000, heartbeatMs = 30_000 } = {}) {
  // Neither <storageRoot>/.locks/ nor <folder>/locks/ is created by any other step.
  ...
  for (;;) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new VfError(ERROR_CODES.DRAFT_LOCKED, 'another operation is in progress for this draft');
      }
...
export async function sweepStaleLocks(storageRoot, { staleMs = 300_000 } = {}) {
  ...
  for (const e of await listDirOrEmpty(storageRoot)) {
    ...
    try { validateFolderCode(e.name); } catch { continue; }   // ignore anything not a folder
