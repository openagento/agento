...
export async function recordAudit(pool, log, storageRoot, row) {
  ...
  const WIDTH = {
    folderCode: 64, operation: 48, draftId: 64, versionId: 64, previousVersion: 64,
  ...
  try {
    ...
    await pool.execute(
      `INSERT INTO versioned_folder_audit
       (folder_code, operation, draft_id, version_id, previous_version, revision,
      ...
      [record.folderCode, record.operation, record.draftId, record.versionId,
    ...
    // operator acts on.
    log?.('versioned_folders', 'ERROR', `audit insert failed: ${errorFacts(err) ?? 'unknown'}`);
    try {
