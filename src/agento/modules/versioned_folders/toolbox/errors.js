export const ERROR_CODES = Object.freeze({
  FOLDER_NOT_FOUND: 'FOLDER_NOT_FOUND', FOLDER_ACCESS_DENIED: 'FOLDER_ACCESS_DENIED',
  DRAFT_NOT_FOUND: 'DRAFT_NOT_FOUND', DRAFT_LOCKED: 'DRAFT_LOCKED',
  DRAFT_HAS_NO_CHANGES: 'DRAFT_HAS_NO_CHANGES', VERSION_NOT_FOUND: 'VERSION_NOT_FOUND',
  VERSION_ALREADY_EXISTS: 'VERSION_ALREADY_EXISTS', FILE_NOT_FOUND: 'FILE_NOT_FOUND',
  INVALID_PATH: 'INVALID_PATH', PATH_OUTSIDE_FOLDER: 'PATH_OUTSIDE_FOLDER',
  SYMLINK_NOT_ALLOWED: 'SYMLINK_NOT_ALLOWED', FILE_TOO_LARGE: 'FILE_TOO_LARGE',
  FOLDER_TOO_LARGE: 'FOLDER_TOO_LARGE', TOO_MANY_FILES: 'TOO_MANY_FILES',
  CURRENT_VERSION_CHANGED: 'CURRENT_VERSION_CHANGED', GIT_OPERATION_FAILED: 'GIT_OPERATION_FAILED',
  FINALIZE_FAILED: 'FINALIZE_FAILED', PUBLISH_FAILED: 'PUBLISH_FAILED',
});
...
export class VfError extends Error {
  ...
  constructor(code, message, options) {
    ...
    this.name = 'VfError'; this.code = code; this.detail = message;
  }
}
 ...
 *  operation's identity reaches the operator through the VfError the caller raises
 ...
 *       through it.
 *    3. `toToolError`, which turns anything that is not a VfError into a generic
 *       GIT_OPERATION_FAILED response. This is the last net, and for a read it is
...
// that is not there". Only these are operator events: FILE_NOT_FOUND and
// DRAFT_NOT_FOUND carry a `cause` too, and logging every legitimate miss buries the
// one line that matters.
const INTERNAL_CODES = new Set([
  ERROR_CODES.GIT_OPERATION_FAILED, ERROR_CODES.FINALIZE_FAILED, ERROR_CODES.PUBLISH_FAILED,
]);
 ...
 *
 *  So what an operator reads for a broken store is the mapped VfError detail plus the
 *  class and the exit code: `GIT_OPERATION_FAILED: the folder store is damaged —
 ...
 *  does not have (see docs/modules/versioned-folders.md).
 *
...
export function toToolError(err, log) {
  ...
  log?.('versioned_folders', 'ERROR', `internal error: ${errorFacts(err) ?? 'unknown'}`);
  return { error_code: ERROR_CODES.GIT_OPERATION_FAILED, message: 'Operation failed' };
}
