...
- **Web** (Python) -- The panel API: sign-in, per-role grants, admin, artifact launches. Holds no secret and never decrypts config; it stores only hashes of session and launch tokens. See [docs/architecture/panel.md](docs/architecture/panel.md).
