...
- **Security:** Toolbox = only container with secrets — enforced per field: `"access": "toolbox_only"` in `system.json` means Python never resolves it (a direct `.get()` raises), and `"allowEnv": false` refuses the `CONFIG__*` source. **Toolbox east-west auth:** every MCP session and every `/api` route needs a **capability token**; the toolbox reads `agent_view_id`/`job_id` from the `toolbox_capability` row, never from a query string or a body, so a caller cannot select a scope it was not issued. Every tool call, on MCP and on `POST /internal/tools/{name}:invoke`, goes through one dispatcher that re-checks the capability, its source and `is_enabled` **per call** and writes a `tool_invocation` audit row first; each token carries `allowed_transports` with no default ([docs/architecture/auth-context.md](docs/architecture/auth-context.md)). A minted token is a credential — stdout once, onward only via stdin or a mode-0600 file, never argv, a log, or shell history ([docs/cli/capability.md](docs/cli/capability.md)). The agent holds NO upstream service credentials — only its own run's capability, scoped to one view and expiring. **`web` and `proxy`** (E1.5): `proxy` (Caddy, `src/agento/framework/docker/proxy/`) is the only container on both `agento-net` and the artifacts `default` network; it strips every caller-supplied `X-Agento-*` and identity header and authenticates its `forward_auth` subrequests with a secret that only `proxy` and `web` can read (volume `proxy-internal`). `web` shares `agento-net` with `sandbox`, so it trusts no identity or forwarding header without that secret — reachability is not a trust signal. The proxy logs redact `cap` and `code` query values. `web` serves the panel API (E2): it stores only SHA-256 hashes of session tokens, launch tokens and exchange codes, and mints one single-use `user_session` capability per panel tool call (see [docs/architecture/panel.md](docs/architecture/panel.md)). Concurrent runs share the `agent` UID and the workspace mount, so a shell-capable agent can read a co-tenant's live capability off disk; closing that needs per-run UID or container isolation (OPEN — see ROADMAP). Tools and skills are **opt-in** (disabled by default) — least privilege: a tool/skill is available only when `is_enabled` resolves to `1` for the scope (agent_view > workspace > default). Adding a module or syncing a skill grants no access until explicitly enabled. Every tool a module can register — including one proxied from an upstream MCP server under a computed name — must be declared in that module's `module.json` `tools[]`; `module:validate` reports an undeclared *literal* `server.tool('x', …)` on a best-effort basis (it skips a source line containing a bare `/`, since division and a regex are indistinguishable without a JS parser — it can miss, never raise a false error), and it strictly enforces globally unique tool names and that a `config.json` `tools/<name>/is_enabled` default belongs to a tool the module declares. The exhaustive check is `src/agento/toolbox/tests/tool-declaration.test.js`, which executes `register()`. A name the gate does not find declared is denied at runtime, and `registerTools` logs a `drift WARN` for anything a module registers without declaring (the backstop for names computed at runtime, e.g. upstream MCP passthrough). By convention (not checkable by the validator) gate each registration on its **own** `tools/<name>/is_enabled` key, and **never** gate on an `is_enabled` key at module level — declare a shared switch and point each member at it with `requires`, so the framework applies it per tool and admin can show a blocked child. Never add a second allow-list mechanism beside `is_enabled`. See [docs/tools/adding-a-tool.md](docs/tools/adding-a-tool.md).
...
- **Versioned artifacts:** the `versioned_artifacts` module stores file trees as drafts / immutable
  ...
  **toolbox only** (the agent container has no path to it, and no generic `git` operation on the
  ...
  container). It declares **no `networks:` key** — Compose leaves it on the project
  `default` network, which it shares with `proxy` alone, while every other service names
  `agento-net` — carries no `env_file:`/`environment:`, mounts
  `storage/versioned-artifacts/published` and `app/etc` read-only, and publishes **no host
  port**: `proxy` is the only route to its files — one `networks:` line added for consistency would let every agent in every
