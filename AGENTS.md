...
- **Security:** target model — the toolbox holds tool credentials; the agent holds no tool credential, only its own harness credential in its per-run HOME, plus the SSH private key waiver (D-SSH-1: per-run `ssh-agent`, never on disk). The cron container keeps the credential store in a `root:root 0600` file read by root-owned `drop.py`, so uid `agent` never sees it (DECISIONS.md D-SSH-1 residual channel 6, closed 2026-09-23). Remaining gaps are listed in [docs/architecture/zero-trust.md](docs/architecture/zero-trust.md#known-exceptions-and-debt). Tools and skills are opt-in: available only when `is_enabled` resolves to `1` for the scope (agent_view > workspace > default), and every tool a module can register must be declared in its `module.json` `tools[]`. Never add a second allow-list mechanism beside `is_enabled`. See [docs/tools/adding-a-tool.md](docs/tools/adding-a-tool.md) and [docs/architecture/cron-privileges.md](docs/architecture/cron-privileges.md).
