...
- **Security:** target model — the toolbox holds tool credentials; the agent holds no tool credential, only its own harness credential in its per-run HOME and its own run's toolbox **capability token** (scoped to one agent_view, expiring). The toolbox reads `agent_view_id`/`job_id` from the `toolbox_capability` row, never from the caller. Every tool call, on MCP and on `POST /internal/tools/{name}:invoke`, goes through one dispatcher that re-checks the capability, its source and `is_enabled` **per call** and writes a `tool_invocation` audit row first; each token carries `allowed_transports` with no default ([docs/architecture/auth-context.md](docs/architecture/auth-context.md)). A minted token is a credential — never argv, a log, or shell history ([docs/cli/capability.md](docs/cli/capability.md)). A `system.json` field with `"access": "toolbox_only"` is never resolved by Python, and `"allowEnv": false` refuses the `CONFIG__*` source. Today's gaps (a headless agent inherits the cron env; co-tenant runs share one UID) are listed in [docs/architecture/zero-trust.md](docs/architecture/zero-trust.md#known-exceptions-and-debt). **`web` and `proxy`**: `proxy` (Caddy, `src/agento/framework/docker/proxy/`) is the only container on both `agento-net` and the artifacts `default` network; it strips every caller-supplied `X-Agento-*` and identity header and authenticates its `forward_auth` subrequests with a secret that only `proxy` and `web` can read. `web` shares `agento-net` with `sandbox`, so it trusts no identity or forwarding header without that secret — reachability is not a trust signal. The proxy logs redact `cap` and `code` query values. `web` serves the panel API (E2): it stores only SHA-256 hashes of session tokens, launch tokens and exchange codes, and mints one single-use `user_session` capability per panel tool call (see [docs/architecture/panel.md](docs/architecture/panel.md)). Tools and skills are opt-in: available only when `is_enabled` resolves to `1` for the scope (agent_view > workspace > default). See [docs/tools/adding-a-tool.md](docs/tools/adding-a-tool.md).
